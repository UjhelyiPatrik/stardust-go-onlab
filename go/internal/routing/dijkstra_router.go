...
func (r *DijkstraRouter) RouteToNode(target types.Node) (types.RouteResult, error) {
	if r.node == nil {
		return nil, errors.New("router not mounted")
	}
	if r.node == target {
		return NewPreRouteResult(0, nil), nil
	}
...
func (r *DijkstraRouter) RouteToService(serviceName string) (types.RouteResult, error) {
	if r.node == nil {
		return nil, errors.New("router not mounted")
	}
...
func (r *DijkstraRouter) CalculateRoutingTable() error {
	...
	for len(queue) > 0 {
		...
		r.routes[entry.Target] = routeEntry{
			OutLink: entry.Path[0],
			Route:   NewPreRouteResult(int(entry.Latency), entry.Path),
		}

		// Handle services for the target node
		r.addServicesToRoutes(entry.Target, entry.Latency, entry.Path)
		...
		for _, link := range entry.Target.GetLinkNodeProtocol().Established() {
			...
			if !visited[neighbor] {
				newPath := make([]types.Link, len(entry.Path))
...
func (r *DijkstraRouter) addServicesToRoutes(target types.Node, latency float64, path []types.Link) {
		...
		if _, exists := r.services[service.GetServiceName()]; !exists {
			...
			if len(target.GetLinkNodeProtocol().Links()) > 0 {
				...
				r.services[service.GetServiceName()] = routeEntry{
					...
					Route:   NewPreRouteResult(int(latency), path), // Creating PreRouteResult with latency
				}
			} else {
